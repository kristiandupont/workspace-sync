...
**Purpose**: React adapter over the framework-agnostic store in `../store.ts` and the coordinator in `../tab-coordinator/`. Exports `createWorkspaceProvider`, a factory that loads an initial workspace snapshot, polls for deltas every 10 seconds, and exposes hooks for subscribing to slices of it.
